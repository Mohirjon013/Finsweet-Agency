    ...
    return (
        <section className='containers w-[1240px] px-0 pb-[96px] mb-[96px] bg-[#ECF8F9] '>
            <nav className='w-full mx-auto flex items-center  justify-center gap-[96px] pt-[43px] border-b-4 border-b-[#E0F1F2]'>
                <NavLink preventScrollReset   end className={activeClass}  to={`/career/job/${id}`}>Details</NavLink>
                <NavLink preventScrollReset   className={activeClass} to={`/career/job/${id}/requirmeents`}>Requirements</NavLink>
                <NavLink preventScrollReset   className={activeClass} to={`/career/job/${id}/responsibilities`}>Responsibilities</NavLink>
            </nav>
            <Outlet/>
        </section> 
    )
